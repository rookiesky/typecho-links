...
//		$form->addInput($user);
		...
		/** 排序 */
        $user = new Typecho_Widget_Helper_Form_Element_Text('order', NULL, 10, _t('排序'), _t('数字越小越靠前'));
        $form->addInput($user);
	...
	public static function output_str($pattern=NULL, $links_num=0, $sort=NULL)
	{
		...
		if (!isset($pattern) || $pattern == "" || $pattern == NULL || $pattern == "SHOW_TEXT") {
			$pattern = "<li><a href=\"{url}\" title=\"{title}\" target=\"_blank\" class=\"text-muted\">{name}</a></li>\n";
